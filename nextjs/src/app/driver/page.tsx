...
export function DriverPage() {
  ...
  useEffect(() => { //no render inicial faz o connect com o socket
    socket.connect();
    return () => {
      socket.disconnect();
    };
  ...
  async function startRoute() {
    ...
    for (const step of steps) { // percorre cada um dos pontos da rota
      await sleep(2000);
      map?.moveCar(routeId, step.start_location);
      socket.emit("new-points", { // vai emitir a atualizaçao do carrinho no mapa
        route_id: routeId,
